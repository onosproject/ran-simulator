...
func (s *Server) Watch(request *metricsapi.WatchRequest, server metricsapi.MetricsService_WatchServer) error {
	...
	for metricsEvent := range ch {
		...
		response := &metricsapi.WatchResponse{
			Metric: metricToAPI(metrics.EntityID(key), metrics.MetricName(key), metricsEvent.Value),
			Type:   eventType(metricsEvent.Type.(metrics.MetricEvent)),
		}
