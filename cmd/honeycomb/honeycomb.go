...
func getHoneycombTopoCommand() *cobra.Command {
	...
	cmd.Flags().StringSlice("service-models", []string{"kpm/1", "ni/2", "rc/3", "kpm2/4", "mho/5"}, "List of service models supported by the nodes")
	...
	cmd.Flags().Uint("uecount", 0, "User Equipment count")
...
func runHoneycombTopoCommand(cmd *cobra.Command, args []string) error {
	...
	ueCount, _ := cmd.Flags().GetUint("uecount")
